...
public record StreamChunk(
    ...
    public StreamChunk(String content, Usage usage) {
        this(content, null, List.of(), List.of(), usage, Map.of(), false);
    }
    ...
    public StreamChunk withContent(String newContent) {
        return new StreamChunk(newContent, reasoning, toolCalls, thinkingBlocks, usage, metadata, terminal);
    }

    /** 生成推理内容不同的新增量块。 */
    public StreamChunk withReasoning(String newReasoning) {
        return new StreamChunk(content, newReasoning, toolCalls, thinkingBlocks, usage, metadata, terminal);
    }

    /** 生成工具调用列表不同的新增量块。 */
    public StreamChunk withToolCalls(List<ToolCall> newToolCalls) {
        return new StreamChunk(content, reasoning, newToolCalls, thinkingBlocks, usage, metadata, terminal);
    }
    ...
    public StreamChunk withMetadata(String key, Object value) {
        ...
        return new StreamChunk(content, reasoning, toolCalls, thinkingBlocks, usage, merged, terminal);
    }

    /** 将该增量块标记为终止块。 */
    public StreamChunk asTerminal() {
        return new StreamChunk(content, reasoning, toolCalls, thinkingBlocks, usage, metadata, true);
    }
}
