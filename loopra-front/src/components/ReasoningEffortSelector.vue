<template>
  <div class="reasoning-selector">
    ...
    <div v-if="open" class="reasoning-menu" :style="{'--progress': `${progress}%`}">
      <div class="summary"><strong>{{ selected.label }}</strong><span>{{ selected.description }}</span></div>
      <div class="track"><input v-model.number="index" aria-label="思考强度" type="range" min="0" max="4" step="1" @change="commit" /></div>
      <div class="levels"><button v-for="(option, optionIndex) in options" :key="option.value" type="button" :class="{active: optionIndex === index}" @click="pick(optionIndex)">{{ option.label }}</button></div>
...
const options = [{value:'none', label:'无', description:'直接响应'}, {value:'low', label:'低', description:'快速响应'}, {value:'medium', label:'中', description:'速度与深度兼顾'}, {value:'high', label:'高', description:'更充分地思考'}, {value:'max', label:'最大', description:'优先获得最完整的推理'}]
const index = ref(4)
