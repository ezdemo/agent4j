<template>
  <section class="model-channels" :data-theme="theme">
    <header class="model-channels-header">
      <button class="model-channels-back" type="button" title="返回" aria-label="返回" @click="emit('back')">
...
async function save() {
  ...
  saving.value = true
  try {
    const response = await configAPI.updateConfig({ modelChannels: payloadChannels, modelChannelId: active.id, model })
...
@media (max-width: 640px) { .model-channels-header { padding: 0 14px; }.model-channels-header p { display: none; }.model-channels-body { width: calc(100% - 28px); padding-top: 14px; }.model-channel-fields { grid-template-columns: 1fr; gap: 12px; padding: 14px; }.model-channel-models { grid-column: auto; } }
</style>
