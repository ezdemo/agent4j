...
/**
 * Agent4J LSP 技能 —— Solon {@link LspTalent} 的薄包装，实现 {@link SolonToTools} 接口。
 * <p>
 * 通过此包装将 Solon AI 框架已有的 LSP 能力（定义跳转、引用查找、悬停提示、
 * 文档符号、调用层次等）无缝接入 agent4j 的 {@link AgentTool} 体系。
 * </p>
 *
...
@Slf4j
@Component
public class SharedAgent4JLspSkill extends LspTalent {

    /**
     * 无参构造（供 Solon IoC 使用），默认工作区为用户目录。
     */
    public SharedAgent4JLspSkill() {
        this(Paths.get(System.getProperty("user.home"), ".agent4j").toAbsolutePath().toString());
    }

    /**
     * @param workDir 工作区根目录，用于 Language Server 的 rootUri
     */
    private SharedAgent4JLspSkill(String workDir) {
        super(new LspManager(workDir), workDir);
        log.info("Agent4JLspSkill 已创建，workDir={}", workDir);
    }

    /**
     * 复制
     */
    public void copyToAgent4J(LspTalent agent4jLspTalent){
        this.getLspManager().getServerConfigs().forEach(
                (name,server) -> {
                    agent4jLspTalent.getLspManager().registerServer(
                            name,server
                    );
                }
        );
