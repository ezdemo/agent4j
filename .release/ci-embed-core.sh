...
# 默认使用 loopra-web/target/loopra-web-dist.tar.gz（本地 mvn package 产物）；
# CI 中传入 actions/download-artifact 下载的核心包路径。
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TARBALL="${1:-$ROOT_DIR/loopra-web/target/loopra-web-dist.tar.gz}"
DEST="$ROOT_DIR/loopra-front/resources/loopra-core"

if [[ ! -f "$TARBALL" ]]; then
  echo "[ERROR] 未找到核心分发包: $TARBALL" >&2
  echo "        请先构建 loopra-web：mvn -B -ntp -DskipTests -pl loopra-web package" >&2
  exit 1
fi
