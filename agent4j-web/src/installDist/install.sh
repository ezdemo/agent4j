...
TARGET_AGENTS="$TARGET_DIR/agent4j.md"
...
if [ -f "$TARGET_AGENTS" ]; then
    ...
    echo "      Found existing agent4j.md (will be preserved)"
else
    echo "      No existing agent4j.md found"
fi
...
if [ -n "$AGENTS_BACKUP" ]; then
    ...
    rm -f "$AGENTS_BACKUP"
    echo "      Preserved existing agent4j.md"
fi

# 检查 jar 文件是否存在
if [ ! -f "$TARGET_BIN_DIR/agent4j-web.jar" ]; then
    echo "[Error] agent4j-web.jar not found in $TARGET_BIN_DIR"
    exit 1
fi
