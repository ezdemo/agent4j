        ...
        <Transition name="tooltip">
          <div v-if="todoTooltipVisible" class="todo-tooltip"
            ...
            <div v-else class="todo-tooltip-list">
              <div v-for="(todo, i) in incompleteTodos" :key="'i'+i" class="todo-tooltip-item">
                <span class="todo-status-icon" :class="todo.status">
                  <svg v-if="todo.status==='in_progress'" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                  <svg v-else width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>
                </span>
              ...
              <div v-if="completedTodos.length>0" class="todo-completed-section">
                <div class="todo-completed-toggle" @click="showCompleted=!showCompleted">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                       :style="{ transform: showCompleted ? 'rotate(90deg)' : '' }"><polyline points="9 18 15 12 9 6"/></svg>
            ...
            <div v-if="todoStats.total>0" class="todo-tooltip-footer">
              <div class="todo-progress-bar"><div class="todo-progress-fill" :style="{ width: (todoStats.completed/todoStats.total*100)+'%' }"></div></div>
            </div>
          </div>
        </Transition>
      </div>
...
<!-- 计划模式按钮已移除 -->
      ...
      <div class="usage-bar">
        ...
        <span class="usage-context-wrap" :title="'上下文: '+fmt(usage.lastPromptTokens||usage.promptTokens)+' / '+fmt(usage.maxContextTokens)">
          上下文
...
const mergedCommands = computed(() => {
  if (backendCommands.value.length > 0) return backendCommands.value.map(c => ({ cmd: c.cmd, desc: c.desc||'', type: c.type||'system' }))
  return defaultSlashCmds
})

const skillCommands = computed(() => backendSkills.value.map(s => ({ cmd: `/skill:${s.name}`, desc: s.description||'运行 skill', type: 'skill' })))
...
const selectSlashCmd = (cmd) => {
  slashPopupOpen.value = false; slashQuery.value = ''
...
const todoStats = computed(() => {
  ...
  const inProgress = props.todos.filter(t=>t.status==='in_progress').length, pending = props.todos.filter(t=>t.status==='pending').length
...
const handleTodoEnter = () => { clearTimeout(todoTimer.value); todoTimer.value = setTimeout(() => { todoTooltipVisible.value = true; emit('fetchTodos') }, 300) }
const handleTodoLeave = () => { clearTimeout(todoTimer.value); todoTimer.value = setTimeout(() => { todoTooltipVisible.value = false }, 50) }
...
const handleInput = () => {
  ...
  if (m) { slashPopupOpen.value = true; slashQuery.value = m[3]||''; activePopupIdx.value = 0; if (!commandsLoaded.value) { commandsLoaded.value = false; loadCommands() } }
  else slashPopupOpen.value = false
}

const handleBlur = () => { inputFocused.value = false; setTimeout(() => { slashPopupOpen.value = false }, 200) }

const handleKeydown = (e) => {
  if (slashPopupOpen.value) {
    if (e.key === 'ArrowDown') { e.preventDefault(); activePopupIdx.value = (activePopupIdx.value+1) % filteredSlashCmds.value.length; return }
    if (e.key === 'ArrowUp') { e.preventDefault(); activePopupIdx.value = (activePopupIdx.value-1+filteredSlashCmds.value.length) % filteredSlashCmds.value.length; return }
    if (e.key === 'Escape') { e.preventDefault(); slashPopupOpen.value = false; return }
    if ((e.key==='Enter' && !e.shiftKey) || e.key==='Tab') { e.preventDefault(); if (filteredSlashCmds.value.length>0) selectSlashCmd(filteredSlashCmds.value[activePopupIdx.value]); return }
  }
  if (e.key==='Escape' && props.streaming) { e.preventDefault(); emit('abort'); return }
...
.send-btn { width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; border-radius: var(--r); color: var(--fg-4); transition: all var(--t); }
...
/* TODO */
.todo-trigger { position: relative; flex-shrink: 0; }
.todo-btn { width: 28px; height: 28px; border-radius: var(--r-sm); display: flex; align-items: center; justify-content: center; color: var(--fg-4); transition: all var(--t); position: relative; cursor: pointer; }
.todo-btn:hover { background: var(--bg-3); color: var(--fg-2); }
.todo-btn.has-todos { color: var(--accent); }
.todo-badge { position: absolute; top: -4px; right: -4px; min-width: 14px; height: 14px; padding: 0 3px; background: var(--accent); color: white; font-size: 9px; font-weight: 700; border-radius: 7px; display: flex; align-items: center; justify-content: center; }
.todo-tooltip { position: absolute; bottom: calc(100% + 8px); left: 0; width: 280px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--r); box-shadow: var(--shadow); z-index: 100; overflow: hidden; }
.todo-tooltip-header { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; background: var(--bg-2); border-bottom: 1px solid var(--border); }
.todo-tooltip-title { font-size: 12px; font-weight: 600; color: var(--fg-2); }
.todo-tooltip-stats { font-size: 11px; color: var(--fg-4); background: var(--bg-3); padding: 2px 6px; border-radius: var(--r-sm); }
.todo-tooltip-empty { padding: 16px; text-align: center; color: var(--fg-4); font-size: 12px; }
.todo-tooltip-list { max-height: 200px; overflow-y: auto; padding: 4px; }
.todo-tooltip-item { display: flex; align-items: center; gap: 6px; padding: 6px 8px; border-radius: var(--r-sm); transition: background var(--t); }
.todo-tooltip-item:hover { background: var(--bg-2); }
.todo-status-icon { flex-shrink: 0; font-size: 11px; line-height: 1.5; }
.todo-content { font-size: 12px; color: var(--fg-2); line-height: 1.5; }
.todo-content.completed { text-decoration: line-through; color: var(--fg-4); }
.todo-content.in_progress { color: var(--accent); font-weight: 500; }
.todo-tooltip-footer { padding: 6px 12px 8px; border-top: 1px solid var(--border); }
.todo-progress-bar { height: 3px; background: var(--bg-3); border-radius: 2px; overflow: hidden; }
.todo-progress-fill { height: 100%; background: var(--accent); border-radius: 2px; transition: width 0.3s ease; }
.todo-completed-section { border-top: 1px solid var(--border); margin-top: 4px; }
.todo-completed-toggle { display: flex; align-items: center; gap: 6px; padding: 6px 8px; font-size: 11px; color: var(--fg-4); cursor: pointer; transition: all var(--t); }
.todo-completed-toggle:hover { color: var(--fg-2); background: var(--bg-2); }
.todo-completed-toggle svg { transition: transform 0.2s ease; }
.collapse-enter-active, .collapse-leave-active { transition: all 0.2s ease; max-height: 200px; }
...
/* 斜杠命令弹窗 */
.slash-popup { position: absolute; bottom: 100%; left: 16px; right: 16px; background: var(--bg); border: 1px solid var(--border); border-radius: var(--r); box-shadow: 0 8px 32px rgba(0,0,0,0.15); z-index: 100; overflow: hidden; margin-bottom: 4px; }
.slash-popup-header { display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; background: var(--bg-2); border-bottom: 1px solid var(--border); }
.slash-popup-title { font-size: 12px; font-weight: 600; color: var(--fg-2); }
.slash-popup-hint { font-size: 11px; color: var(--fg-4); }
.slash-popup-list { max-height: 280px; overflow-y: auto; padding: 4px; }
.slash-popup-loading, .slash-popup-empty { padding: 24px 16px; text-align: center; font-size: 13px; color: var(--fg-4); display: flex; align-items: center; justify-content: center; gap: 8px; }
.slash-popup-loading { color: var(--accent); }
.slash-popup-item { display: flex; align-items: center; gap: 8px; padding: 6px 10px; border-radius: var(--r-sm); cursor: pointer; transition: background var(--t); }
.slash-popup-item:hover, .slash-popup-item.active { background: var(--bg-2); }
.slash-popup-icon { width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; font-size: 14px; background: var(--bg-3); border-radius: var(--r-sm); flex-shrink: 0; }
.slash-popup-info { flex: 1; min-width: 0; }
.slash-popup-cmd { font-size: 13px; font-weight: 600; font-family: var(--mono); color: var(--fg); display: flex; align-items: center; gap: 4px; }
.slash-popup-badge { display: inline-flex; align-items: center; padding: 0 4px; font-size: 10px; font-weight: 500; border-radius: var(--r-sm); line-height: 1.4; }
.slash-popup-badge.skill { background: #dcfce7; color: #16a34a; }
.slash-popup-badge.mode { background: #dbeafe; color: #2563eb; }
.slash-popup-badge.session { background: #fef3c7; color: #d97706; }
.slash-popup-desc { font-size: 11px; color: var(--fg-4); margin-top: 1px; }
[data-theme="dark"] .slash-popup-badge.skill { background: #052e16; color: #4ade80; }
[data-theme="dark"] .slash-popup-badge.mode { background: #1e3a5f; color: #60a5fa; }
[data-theme="dark"] .slash-popup-badge.session { background: #422006; color: #fbbf24; }
...
.usage-value.high { color: var(--red); font-weight: 600; }
.usage-value.medium { color: var(--yellow); }
.usage-refresh { width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; border-radius: var(--r-sm); color: var(--fg-4); font-size: 14px; transition: all var(--t); cursor: pointer; }
