...
/**
 * 会话级工作树隔离服务 —— 为开启"工作树隔离模式"的会话创建独立 git worktree。
 * <p>
 * 设计要点：
 * <ul>
 *   <li>工作树位于 {@code <worktreeBaseDir>/<workspaceHash>-<sessionName>}（默认 ~/.loopra/worktree/），
 *       检出独立分支 {@code loopra/sandbox-<wsHash8>-<sessionName>}（git 禁止两个 worktree 同分支）。</li>
 *   <li>会话身份（JSONL 历史/Goal/Checklist）仍绑定主工作区，仅 AI 的文件操作落在工作树。</li>
 *   <li>合并回主工作区采用<b>工作树内反向合并</b>：先在 worktree 中 {@code git merge <主分支>}，
 *       冲突留在工作树内由 AI/用户解决（不突破工具路径边界）；干净后在主工作区
 *       {@code git merge --ff-only <工作树分支>} 快进，随后清理工作树与分支。</li>
 * </ul>
 *
 * @author Sorghum
 */
...
public class WorktreeService {
    ...
    /**
     * 启动时清理孤儿工作树：baseDir 下未被任何已注册工作区引用（git worktree 列表）
    ...
    public void cleanupOrphanWorktrees() {
        try {
            ...
            for (WorkspaceManager.WorkspaceInfo ws : new WorkspaceManager().listWorkspaces()) {
            ...
            try (Stream<Path> children = Files.list(base)) {
                for (Path child : children.filter(Files::isDirectory).toList()) {
                    ...
                    if (!registered.contains(normalized)) {
                        ...
                        log.info("[worktree] 已清理孤儿工作树目录: {}", child);
                    }
                }
            }
        } catch (Exception e) {
            log.warn("[worktree] 孤儿工作树清理失败（不影响启动）: {}", e.getMessage());
        }
    }
    ...
    /** 工作树根目录（全局配置 worktreeBaseDir，默认 ~/.loopra/worktree；可用系统属性 loopra.worktree.baseDir 覆盖，供测试/嵌入方使用）。 */
    public Path baseDir() {
    ...
    /** 会话工作树磁盘路径：&lt;base&gt;/&lt;workspaceHash&gt;-&lt;sessionName&gt;。 */
    ...
    /**
     * 查询会话工作树状态。主工作区非 Git 仓库时返回 exists=false 及说明，不抛异常。
     */
    public WorktreeStatusDTO status(String workspaceHash, String sessionName) {
        File main = mainWorkspace(workspaceHash);
        if (main == null) {
            return new WorktreeStatusDTO(workspaceHash, sessionName, false, null, null, false, null, "未设置工作区");
        }
    ...
    public EnvironmentStatusDTO environment(String workspaceHash, String sessionName) {
        ...
        if (main == null) {
            return new EnvironmentStatusDTO(workspaceHash, sessionName, "unavailable",
                    null, null, false, null, null, false, false, false, "未设置工作区");
        }
        ...
        if (!worktreeMode) {
            return new EnvironmentStatusDTO(resolvedHash, sessionName, "local",
                    ...
                    main.getPath(), mainBranch, mainDirty, false, agentRunning, "当前 Agent 使用本地工作区");
        }
        ...
        if (!exists) {
            return new EnvironmentStatusDTO(resolvedHash, sessionName, "worktree",
                    main.getPath(), mainBranch, mainDirty,
                    null, null, false, false, agentRunning, "工作树将在会话首次运行时创建");
        }
        ...
        return new EnvironmentStatusDTO(resolvedHash, sessionName, "worktree",
                ...
                currentDirty ? "Agent 正在使用隔离工作树" : "隔离工作树干净");
    }
    ...
    WorktreeStatusDTO statusAt(String workspacePath, String sessionName) {
        ...
        return new WorktreeStatusDTO(wsHash, sessionName, exists,
                ...
                exists ? (dirty ? "工作树有未提交改动" : "工作树干净") : "工作树未创建");
    }

    /**
     * 创建会话工作树（幂等：已存在时直接返回状态）。
     * 基于主工作区当前 HEAD 创建独立分支并检出。
     */
    ...
    WorktreeStatusDTO createAt(String workspacePath, String sessionName) {
        ...
        try {
            ...
            if (result.exitCode != 0) {
                throw new ServiceException("创建工作树失败: " + result.stderr);
            }
            // 软链主工作区的 .codegraph 索引（Junction/符号链接），让 codegraph_explore 等
            // 依赖索引的技能在工作树内可用；失败时不影响功能（AI 可显式传主工作区路径）。
            linkCodegraphIndexes(main, wt);
            log.info("[worktree] 已创建会话工作树: {} (分支 {})", wt, branch);
            return statusAt(workspacePath, sessionName);
        } catch (ServiceException e) {
            throw e;
        } catch (Exception e) {
            throw new ServiceException("创建工作树失败: " + e.getMessage());
        }
    }
     ...
     * 删除会话工作树及其分支。
     *
     * @param discardChanges 为 true 时强制删除（丢弃工作树内所有未提交/未合并改动）
     */
    public WorktreeStatusDTO remove(String workspaceHash, String sessionName, boolean discardChanges) {
        ...
        if (main == null || !Files.exists(wt)) {
            return new WorktreeStatusDTO(wsHash, sessionName, false, wt.toString(), null, false, null, "工作树不存在");
        }
        return removeAt(main.getPath(), sessionName, discardChanges);
    }

    /** 按工作区路径删除工作树（供测试与内部复用）。 */
    WorktreeStatusDTO removeAt(String workspacePath, String sessionName, boolean discardChanges) {
        ...
        try {
            ...
            if (result.exitCode != 0) {
                throw new ServiceException("删除工作树失败: " + result.stderr);
            }
            ...
            throw new ServiceException("删除工作树失败: " + e.getMessage());
        }
    }

    /**
     * 将工作树分支合并回主工作区。
     * <p>流程：工作树内反向合并主分支（冲突留在工作树内）→ 干净则主工作区 ff-only 快进。
     * 有冲突时保留工作树并返回冲突文件列表，供 AI/用户解决后再次调用。</p>
     */
    ...
    WorktreeMergeResultDTO mergeBackAt(String workspacePath, String sessionName) {
        ...
        if (mainBranch == null || "HEAD".equals(mainBranch)) {
            throw new ServiceException("主工作区处于 detached HEAD 状态，无法合并");
        }
        String worktreeBranch = branchFor(wsHash, sessionName);

        try {
            // 1. 工作树有未提交改动时自动提交（保持 AI 无需关心 git 细节）；
            //    元数据目录（.loopra）以主分支版本为准，避免记忆/共享工作区反向覆盖主工作区
            if (hasChanges(main, wt)) {
                ...
                if (add.exitCode != 0) {
                    throw new ServiceException("工作树暂存失败: " + add.stderr);
                }
                ...
                if (commit.exitCode != 0 && !commit.stderr.contains("nothing to commit")) {
                    throw new ServiceException("工作树提交失败: " + commit.stderr);
                }
            }

            // 2. 工作树内反向合并主分支（冲突留在工作树，不触碰主工作区）
            ...
            if (reverse.exitCode != 0) {
                ...
                return new WorktreeMergeResultDTO(false, true, conflicts, null, false,
                        "合并冲突，需在工作树内解决后再次合并；冲突文件数: " + conflicts.size());
            }

            // 3. 主工作区必须干净才能 ff-only 快进
            if (hasChanges(main, main.toPath())) {
                throw new ServiceException("主工作区有未提交改动，请先提交或使用 git stash 暂存后再合并");
            }
            ProcessResult ff = gitService.runGit(main, WORKTREE_TIMEOUT_SEC,
                    "git", "merge", "--ff-only", worktreeBranch);
            if (ff.exitCode != 0) {
                throw new ServiceException("主工作区快进合并失败: " + ff.stderr);
            }
            String head = simple(main, "git", "rev-parse", "HEAD");

            // 4. 保留工作树与分支，供同一会话继续隔离修改和后续再次合并。
            //    会话删除时由 AgentService 统一清理工作树与分支。
            log.info("[worktree] 会话工作树已合并回主工作区并保留: {}/{} -> {}", wsHash, sessionName, head);
            return new WorktreeMergeResultDTO(true, false, List.of(), head, false,
                    "已合并回主工作区，工作树继续保留");
        } catch (ServiceException e) {
            throw e;
        } catch (Exception e) {
            throw new ServiceException("合并工作树失败: " + e.getMessage());
        }
    }
    ...
    private File requireMainWorkspace(String workspaceHash) {
        File main = mainWorkspace(workspaceHash);
        if (main == null) throw new ServiceException("未设置工作区");
        if (!isGitRepo(main)) {
            throw new ServiceException("工作区不是 Git 仓库，无法使用工作树隔离模式");
        }
        return main;
    }

    /** 按路径校验主工作区（供测试与路径级内部方法使用）。 */
    private File requireMainWorkspacePath(String workspacePath) {
        File main = new File(workspacePath);
        if (!main.isDirectory()) throw new ServiceException("工作区不可访问: " + workspacePath);
        if (!isGitRepo(main)) {
            throw new ServiceException("工作区不是 Git 仓库，无法使用工作树隔离模式");
        }
        return main;
    ...
    /** 工作区路径 hash（与 WorkspaceManager 一致，供路径级方法使用）。 */
    static String hashOf(String workspacePath) {
        return workspacePath == null ? "" : site.sorghum.loopra.bin.workspace.WorkspaceManager.computeHash(workspacePath);
    }
    ...
    /**
     * 工作树内 .loopra 元数据目录（项目记忆/共享工作区）以主分支版本为准：
     * 合并回主工作区时不允许工作树内的记忆/协作数据反向覆盖主工作区。
     */
    ...
    /**
     * 将主工作区的 .codegraph 索引以符号链接/Junction 形式带入工作树，
