...
public class InitCommand implements ChatCommand {
    ...
    public String getDescription() {
        return "/init        自动分析项目生成 loopra.md";
    }
    ...
    public CommandResult execute(MessageWrapper input, ChatCommandContext context) {
        ...
        String prompt = """
                请全面分析这个项目的代码库，生成 loopra.md 放在项目根目录。
                
                要求：
                1. 用 tree / glob 了解项目结构
                2. 阅读核心源文件
                3. loopra.md 应包含：项目概述、目录结构树、技术栈表格、架构设计、全部工具列表、运行方式
                4. 用 write_file 写入 loopra.md
                5. 完成后一句话总结""";
        try {
