/**
 * Agent4jApp —— 连接 agent4j 后端的终端 AI 代理界面
 *
 * 通过 HTTP REST + SSE 流式桥接 agent4j 后端的所有能力：
...
export interface Agent4jAppProps {
    /** agent4j 后端地址，如 http://localhost:8097 */
    apiUrl: string;
    /** 可选的工作区哈希 */
...
export function Agent4jApp({apiUrl, workspaceHash: defaultWorkspace, token}: Agent4jAppProps): React.ReactElement {
    ...
    const [appTitle, setAppTitle] = useState("Agent4j");
    ...
    // ── agent4j 专用斜杠命令 ─────────────────────────────────
    const handleAgent4jSlash = useCallback((cmd: string, args: string[]): boolean => {
        switch (cmd) {
            case "new":
            case "clear":
            case "reset": {
    ...
    }, [dispatch, workspace, slashCtx, handleAgent4jSlash]);
